...
| `APP_VERSION` | No | file fallback or `dev` | string | No | Yes | Fallback only; `/app/VERSION` is authoritative when present. |
| `APP_CHANNEL` | No | empty | string | No | Yes | Fallback for `/app/VERSION_CHANNEL`. |
